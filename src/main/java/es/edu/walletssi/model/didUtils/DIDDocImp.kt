...
data class DIDDocImp(
    ...
    companion object {
        fun fromJson(doc: String): DIDDocImp = DIDDocDecoder.decodeJson(doc)
    }
...
object DIDDocEncoder {
     ...
     */
    fun encodeJson(doc: DIDDocImp, pretty: Boolean = false): String {
        ...
        if (doc.verificationMethods.isNotEmpty()) {
            val verificationMethod = doc.verificationMethods.fold(JsonArray()) { arr, el -> arr.add(
                encodeVerificationMethod(el)
            ); arr }
...
object DIDDocDecoder {
     ...
     * https://www.w3.org/TR/did-core/#did-document-properties
     */
    fun decodeJson(doc: String): DIDDocImp {
