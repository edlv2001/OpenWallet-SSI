...
public class VerifiableCredentialIssuer {
    ...
    public VerifiableCredential issue(@NotNull String schema, String type, Map<String, Object> claims, String didIssuer, String didHolder, Date expirationDate, boolean store){
        ...
        if(store)
            store(vc, created);
        //vc.addToJsonLDObject(, "credentialSchema");
        //System.out.println(vc.getJsonObject().get("credentialSchema"));
        return vc;
    }
    ...
    private void store(VerifiableCredential vc, File dir){
        ...
        File file = new File(dir, "VerifiableCredential_" + hash(vc));
        if(file.exists()){
    ...
    public static String hash(JsonLDObject jsonLDObject){
        return Hex.encode(
                KeyUtils.hashSHA256(
                        jsonLDObject.toJson(false).getBytes(StandardCharsets.UTF_8)
                )
        );
    }
